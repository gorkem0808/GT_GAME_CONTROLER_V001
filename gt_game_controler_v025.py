...
APP_TITLE = "GT GAME CONTROLER v025"
        ...
        try:
            for i, p in enumerate(parts):
                ...
                if p == "TIMEOUT" and i+1 < len(parts):
                    self.timeout_sec = int(parts[i+1])
        except Exception:
            pass
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.timeout_sec = tk.StringVar(value="180")
    ...
    def create_ui(self):
        ...
        ttk.Label(header, text="v025  |  Süre ayarı eklendi", style="Sub.TLabel").pack(side="left", padx=16)
    ...
    def make_controller_tab(self):
        ...
        ttk.Label(top,text="Pasif süre (sn):").grid(row=4,column=0,sticky="e",padx=5,pady=5)
        self.timeout_entry = ttk.Entry(top,textvariable=self.timeout_sec,width=12)
        self.timeout_entry.grid(row=4,column=1,sticky="w")
        ttk.Label(top,text="Örnek: 60=1 dk, 180=3 dk, 300=5 dk").grid(row=4,column=2,columnspan=2,sticky="w",padx=5)
        ...
        ttk.Button(top,text="Controller Config Oku",command=self.read_controller_config).grid(row=0,column=6,rowspan=5,padx=10,sticky="ns")
        ...
        self.timeout_entry.bind("<KeyRelease>", lambda e: self.mark_controller_dirty())
    ...
    def apply_controller_values_from_device(self, kb, force=False):
        ...
        self.timeout_sec.set(str(getattr(kb, 'timeout_sec', 180)))
    ...
    def send_cfg(self):
        ...
        try:
            timeout_s = int(self.timeout_sec.get())
        except Exception:
            messagebox.showwarning("Hatalı süre", "Pasif süre saniye olarak sayı olmalı. Örnek: 180"); return
        if timeout_s < 10: timeout_s = 10
        if timeout_s > 3600: timeout_s = 3600
        self.timeout_sec.set(str(timeout_s))
        ...
        self.expected_cfg = (p1, p2, c1, c2, t1, t2, timeout_s, self.relay_mode.get(), self.p1_coin.get(), self.p2_coin.get())

        cmds = [
            f"SETCFG,{self.relay_mode.get()},{self.p1_coin.get()},{self.p2_coin.get()},{p1},{p2},{c1},{c2},{t1},{t2},{timeout_s}",
            f"RELAYPIN,1,{p1}",
            f"RELAYPIN,2,{p2}",
            f"COINPIN,1,{c1}",
            f"COINPIN,2,{c2}",
        ...
        for cmd in cmds:
            d.send(cmd)
            time.sleep(0.06)
        self.ctrl_status.set(f"Kaydediliyor: P1 coin GP{c1}, P2 coin GP{c2}, P1 tetik GP{t1}, P2 tetik GP{t2}, P1 röle GP{p1}, P2 röle GP{p2}, süre {timeout_s} sn")
    ...
    def verify_controller_config(self):
        ...
        if not d or not self.expected_cfg:
            self.controller_fields_dirty = False
            return
        p1, p2, c1, c2, t1, t2, timeout_s, rm, pc1, pc2 = self.expected_cfg
        ok = (d.p1_relay_pin == p1 and d.p2_relay_pin == p2 and
              getattr(d, 'p1_coin_pin', 17) == c1 and getattr(d, 'p2_coin_pin', 21) == c2 and
              getattr(d, 'p1_trigger_pin', 7) == t1 and getattr(d, 'p2_trigger_pin', 11) == t2 and
              getattr(d, 'timeout_sec', 180) == timeout_s)
        self.controller_fields_dirty = False
        self.config_apply_lock_until = 0
        if ok:
            self.ctrl_status.set(f"✅ Ayar kaydedildi: P1 coin GP{c1}, P2 coin GP{c2}, P1 tetik GP{t1}, P2 tetik GP{t2}, P1 röle GP{p1}, P2 röle GP{p2}, süre {timeout_s} sn")
            ...
            messagebox.showwarning("Ayar kaydedilmedi",
                ...
                "Bu genelde Controller Pico'da eski UF2 yüklü demektir. Yeni V025 controller.uf2 derlenip yüklenmeli.")
    ...
    def refresh_ui(self):
        ...
        if kb:
            ...
            self.relay_text.insert("end",f"P1 tetik GP{getattr(kb, 'p1_trigger_pin', 7)} | P2 tetik GP{getattr(kb, 'p2_trigger_pin', 11)} | Süre: {getattr(kb, 'timeout_sec', 180)} sn\n")
